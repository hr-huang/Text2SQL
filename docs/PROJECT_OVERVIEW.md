...
**黄金评测集**：68 道手工标注的题 + 标准 SQL + 难度分级
- simple（29）：直接查
- medium（29）：多表 JOIN / 子查询 / 窗口函数
- complex（10）：**多跳查询**——必须先拿到中间结果，再基于中间结果查下一步

> complex 的判定标准是「是否存在必须传递的中间结果集」，不是「SQL 写得复不复杂」。
> 含子查询、CTE、窗口函数但一条 SQL 能写完的，一律不算 complex。

**自动对比**：跑 AI 的 SQL → 跟标准 SQL 跑出来的结果对比（行集合是否相等）。
浮点数按 2 位小数容差比对（`14.125` 与 `14.13` 视为相同），避免精度误判。

**复杂题判分（两层，LLM-as-judge 兜底）**：
1. **规则层**（免费、可复现）：路由验证（必须走 `decompose→orchestrator`）+ 子任务全部执行成功 + 结果集等价比对
2. **judge 层**（只有规则层不过才调 LLM）：把「问题 + 生成答案 + gold 答案」交给独立 LLM
   判断是否**语义等价**——容忍列名不同、明细 vs 聚合、ID vs 名称、行序不同。
   这是 Spider 2.0 一类的业界做法；纯规则判分会把"语义对但格式不同"的答案误判成错。

> 评测方法论踩过的坑（都修掉了）：
> - 并发跑评测会触发 API 连接错误，`Connection error` 污染整轮数据 → **必须串行跑**
> - 单次跑受 LLM 随机性影响（同一题两次可能不同）→ 关键结论要多跑几次看趋势
> - 判分"值集合匹配"比"按列位置比对"稳健——列名不匹配时位置比对必然误判
...
> **Complex 判分的口径演进（面试可讲）**：最早 complex 只统计「子问题 SQL 是否执行成功」，
> `gold_sql` 根本没参与比对——这不测正确性，只测「能不能跑通」。后来改成三重校验
> （路由 / 子任务执行 / 答案与 gold_sql 比对）。
>
> ⚠️ **残留的已知局限**：多跳题的中间步骤口径不唯一。例如「退货率最高的3个品类」，
> 用 `COUNT(DISTINCT 退货单)/COUNT(DISTINCT 订单)` 和 `SUM(退货数量)/SUM(销售数量)`
> 会算出不同的 Top3，导致最终答案不同但**两种算法都合理**。
> 所以 complex 的「答案匹配率」是**严格下界**，不是真实正确率——需要人工逐题核验
> 才能区分「系统真的错了」和「只是口径不同」。
>
> 实测中严格匹配率低但人工核验后大多只是**表达差异**，典型三类：
> - **聚合粒度**：gold 要 `GROUP_CONCAT` 聚合成一行，LLM 给「客户×品类」明细行（语义等价）
> - **实体标识**：LLM 返回 `product_id`，gold 返回 `product` name
> - **浮点精度**：LLM 给 `14.125`，gold 写 `ROUND(...,2)` 得 `14.13`
>
> 为此 `compare_results` 加了数值容差（统一 2 位小数），但前两类无法自动化解。

#### 一个反直觉的发现：多跳 ≠ 必须编排

最初假设「complex 题必须走 decompose→orchestrator」。实测推翻了它——
**在单库 27 表的 Text2SQL 场景下，绝大多数多跳问题都能用一条 CTE + 窗口函数的 SQL 写完**。
分类器给出的理由很扎实：

> 「各品类中销售额排名前3的商品，它们的月均销量」→ 可用一条 SQL 通过窗口函数
> 按品类排名后筛选前3，再聚合计算月均销量，无需多步中间结果。

所以 `orchestrator` 的价值**不是"必需"**，而是：
1. 降低单步 SQL 复杂度，每步更容易写对
2. 每步结果可独立验证，出错只影响一步
3. 上下文显式传递，避免超长 SQL 里的隐式耦合

为此加了 `--force-decompose` 评测开关：绕过 LLM 的智能判断，强制走编排路径，
这样能分别衡量「智能路由」和「强制编排」两条路径的成功率，而不是笼统报一个数。

> ⚠️ **不要为了用编排而用编排**。先量清楚它在你的数据规模下到底带来什么，
> 这是这个项目反复验证的一个判断。
